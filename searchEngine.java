...
public class searchEngine {
	...
	void traverseInternet(String url) throws Exception {
		...
		while (itr1.hasNext()) { /*
															 * while there are adjacent vertices starting from first element in the linked
															 * list
															 */
	...
	void computePageRanks() {
		...
		for (int i = 0; i < 100; i++) { /* for 100 iterations */
			...
			while (itr1.hasNext()) { /* looping through all vertices */
				...
				LinkedList<String> edgesIntoList = internet
						.getEdgesInto(vertex); /* linked list of all the edges into the current vertex */
				...
				while (itr2.hasNext()) {
					...
					pageRank += (0.5 * (internet.getPageRank(edgeInto)
							/ internet.getOutDegree(edgeInto))); /* adding the page rank contribution of each new edge */
				}
